...
NSInteger UIKBType = UIKeyboardTypeNumbersAndPunctuation;


-(IBAction)solveProblem:(id)sender 
	{
	double varA = [varAfield.text doubleValue]; //try to create a new method which can handle fractions
    ...
    NSString *maxMin = [[NSString alloc] initWithFormat:@""];    
...
// If methods to do math work and display text.
	if (varA == 0) 
	{
		...
		else if (varDisc < 0) //solve the quadratic equation using varDisc, else if used to prevent roots of imaginary numbers.
		{
			...
			NSString *String = [[NSString alloc] initWithFormat:@"Roots: \n%lf + %lfi or\n%lf - %lfi\n\nDiscriminant: \n%lf\n\nVertex:\n(%lf, %lf)\n\nAxis of Symmetry:\nX = %lf\n\nType of Symmetry:\n%@\n\nGraph Opens: %@\n\nVertex is: %@\n\nDerivative:\n%lfx + %lf", varI1, varI2, varI1, varI2, varDisc, coorX, coorY, coorX, symm, opening, maxMin, deriv, varB];
		...
		else if(varDisc > 0) 
		{
			...
			NSString *String = [[NSString alloc] initWithFormat:@"Roots: \n%lf or %lf\n\nDiscriminant: \n%lf\n\nVertex:\n(%lf, %lf)\n\nAxis of Symmetry:\nX = %lf\n\nType of Symmetry:\n%@\n\nGraph Opens: %@\n\nVertex is: %@\n\nDerivative:\n%lfx + %lf", varX1, varX2, varDisc, coorX, coorY, coorX, symm, opening, maxMin, deriv, varB];
			answerLabel.text = String;
			answerLabel.textColor = [UIColor whiteColor];
			answerLabel.textColor = [UIColor blackColor];
			[String release];
		...
		{
			varX1 = ( (-varB + ( sqrt(varDisc)) )/(2*varA) );
			NSString *String = [[NSString alloc] initWithFormat:@"There is only one root: \n%lf\n\nDiscriminant: \n%lf\n\nVertex:\n(%lf, %lf)\n\nAxis of Symmetry:\nX = %lf\n\nType of Symmetry:\n%@\n\nGraph Opens: %@\n\nVertex is: %@\n\nDerivative:\n%lfx + %lf", varX1, varDisc, coorX, coorY, coorX, symm, opening, maxMin, deriv, varB];
			answerLabel.text = String;
			answerLabel.textColor = [UIColor whiteColor];
			answerLabel.textColor = [UIColor blackColor];
			[String release];
